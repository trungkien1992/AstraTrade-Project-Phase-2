...
class EnhancedHapticService {
  ...
  Future<void> triggerCustomPattern(HapticPattern pattern, {double intensity = 1.0}) async {
    if (!_isEnabled || !_isInitialized) return;
  ...
  Future<void> _triggerPattern(HapticPattern pattern, {double intensity = 1.0}) async {
    try {
      final adjustedIntensities = pattern.intensities
          .map((i) => (i * intensity * _intensityMultiplier).round().clamp(0, 255))
          .toList();
  ...
  /// Trigger custom haptic pattern
  Future<void> _triggerCustomPattern(List<int> intensities, List<int> durations) async {

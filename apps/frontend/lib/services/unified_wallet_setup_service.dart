...
class UnifiedWalletSetupService {
  ...
  static Future<User> setupWalletWithTrading({
    ...
    try {
      ...
      debugPrint(
        '   Address: ${starknetAddress.substring(0, 10)}...${starknetAddress.substring(starknetAddress.length - 8)}',
      );
      ...
      final user = User(
        ...
        extendedExchangeApiKey:
            null, // Will be generated when user starts real trading
      );
      ...
      debugPrint(
        '✅ Wallet setup completed successfully with trading capabilities',
      );
      return user;
    } catch (e) {
  ...
  static Future<User> setupFreshWallet({
    ...
    try {
      ...
      final walletServiceAddress = await walletImportService
      ...
      debugPrint(
        '   WalletImportService direct: "$walletServiceAddress" (${walletServiceAddress?.length ?? 0})',
      );
  ...
  static Future<void> _validateStarknetCredentials(
    String privateKey,
    String address,
  ) async {
    ...
    try {
      ...
      debugPrint(
        '🔍 Address validation: "$address" (length: ${address.length})',
      );
      if (address.isEmpty ||
          !address.startsWith('0x') ||
          address.length != 66) {
        debugPrint(
          '❌ Address validation failed: empty=${address.isEmpty}, starts0x=${address.startsWith('0x')}, length=${address.length}',
        );
      ...
      final walletImportService = WalletImportService();
      final derivedAddress = await walletImportService
          .deriveAddressFromPrivateKey(privateKey);
  ...
  static Future<String> _setupExtendedExchangeIntegration(
    ...
    try {
      ...
      debugPrint(
        '⚠️ Extended Exchange API integration requires user to enter their own API key',
      );
      debugPrint(
        '⚠️ Please prompt user to enter their Extended Exchange API key',
      );
      ...
      debugPrint(
        '✅ Extended Exchange integration complete with user-provided key',
      );
      return apiKey;
    } catch (e) {
  ...
  static Future<void> _validateTradingCapability(
    String privateKey,
    String apiKey,
  ) async {
    ...
    try {
      ...
      final isApiKeyValid = await ExtendedExchangeApiService.validateApiKey(
        apiKey,
      );
      if (!isApiKeyValid) {
        debugPrint(
          '⚠️ Extended Exchange API key validation failed - proceeding anyway',
        );
      }
      ...
      try {
        ...
        final testPayload =
            'test_trading_capability_${DateTime.now().millisecondsSinceEpoch}';
        ...
        if (!canSign) {
          throw WalletSetupException(
            'Private key cannot generate valid signatures',
          );
        }
      ...
      debugPrint(
        '✅ Proceeding with wallet creation despite trading validation issues',
      );
    }
  }
    ...
    try {
      ...
      if (apiKey == null) {
        return WalletTradingStatus(
          ...
          hasApiKey: false,
          message:
              'No Extended Exchange API key found. Please enter your API key.',
        );
      }
      ...
      return WalletTradingStatus(
        ...
        message: isValid
            ? 'Trading ready'
            : 'API key invalid. Please enter a valid API key.',
      );
    } catch (e) {
      return WalletTradingStatus(
        isReady: false,
  ...
  static Future<User> importWalletFromPrivateKey({
    ...
    try {
      debugPrint(
        '📥 Importing wallet from private key with mobile-native SDK...',
      );
  ...
  static Future<String> signTransactionForExtendedAPIHybrid({
    ...
    try {
      debugPrint(
        '🔐 Signing transaction for Extended API using mobile service...',
      );

      // Use mobile service to sign the transaction
      final signature = await signTransactionForExtendedAPI(
        orderData: orderData,
      );
...
String _generatePrivateKey() {
  try {
    ...
    } while (privateKeyBigInt >= starknetFieldPrime ||
        privateKeyBigInt == BigInt.zero);

    final validPrivateKey = privateKeyBigInt.toRadixString(16).padLeft(64, '0');
    debugPrint(
      '✅ Generated valid private key within Starknet field (${validPrivateKey.length} chars)',
    );
...
Future<String> _deriveStarknetAddress(String privateKey) async {
  try {
    debugPrint(
      '🏭 Deriving address from private key: ${privateKey.substring(0, 10)}...',
    );
    ...
    if (address != null && address.isNotEmpty) {
      debugPrint(
        '✅ WalletImportService address: "$address" (length: ${address.length})',
      );
      return address;
    }
    ...
    debugPrint(
      '⚠️ WalletImportService returned null, using fallback generator',
    );
    final generator = _StarknetAddressGenerator();
    final fallbackAddress = generator.generateAddress(privateKey);
    debugPrint(
      '✅ Fallback address: "$fallbackAddress" (length: ${fallbackAddress.length})',
    );
    ...
    final fallbackAddress = generator.generateAddress(privateKey);
    debugPrint(
      '✅ Error fallback address: "$fallbackAddress" (length: ${fallbackAddress.length})',
    );
    return fallbackAddress;
  }
}

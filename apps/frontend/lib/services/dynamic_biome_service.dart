...
class DynamicBiomeService {
  ...
  Future<void> updateFromTradingPerformance({
    ...
    try {
      ...
      if (isWin) {
        ...
        _consecutiveLosses++;
        _consecutiveWins = 0;
        _tradingMomentum = (_tradingMomentum - tradingImpact * 0.3).clamp(
          0.0,
          1.0,
        );
        _cultivationEnergy +=
            tradingImpact * 0.2; // Still some energy from learning
      }
  ...
  void _syncWithTradingData() {
    ...
    _visualIntensity = (_visualIntensity + _tradingMomentum * 0.1).clamp(
      0.0,
      1.0,
    );
  }
...
enum BiomeEvolutionType { initialized, progress, evolved, tradingUpdate }

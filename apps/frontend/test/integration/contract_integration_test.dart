...
void main() {
  group('AstraTrade Contract Integration Tests', () {
    ...
    setUpAll(() {
      ...
      testUserAddress = '0x05715B600c38f3BFA539281865Cf8d7B9fE998D79a2CF181c70eFFCb182752F7';
    ...
    group('Contract Address Configuration', () {
      ...
      test('should have valid explorer URLs', () {
        expect(ContractAddresses.exchangeExplorerUrl, contains('sepolia.starkscan.co'));
        expect(ContractAddresses.vaultExplorerUrl, contains('sepolia.starkscan.co'));
        expect(ContractAddresses.paymasterExplorerUrl, contains('sepolia.starkscan.co'));
        
        expect(ContractAddresses.exchangeExplorerUrl, contains(ContractAddresses.exchangeContract));
        expect(ContractAddresses.vaultExplorerUrl, contains(ContractAddresses.vaultContract));
        expect(ContractAddresses.paymasterExplorerUrl, contains(ContractAddresses.paymasterContract));
      });
    });

    group('Exchange Service Integration', () {
      test('should use correct contract address', () {
        expect(AstraTradeExchangeV2Service.CONTRACT_ADDRESS, 
    ...
    group('Paymaster Service Integration', () {
      test('should have proper configuration structure', () {
        expect(() {
          ...
          expect(contractAddress, isNotEmpty);
          debugPrint('Paymaster service configured for contract: $contractAddress');
        }, returnsNormally);
      });
    });
    ...
    group('Integration Readiness', () {
      test('all components should be configured for integration', () {
        ...
        final readinessChecks = <String, bool>{
          'Exchange contract address configured': ContractAddresses.exchangeContract.isNotEmpty,
          'Vault contract address configured': ContractAddresses.vaultContract.isNotEmpty,
          'Paymaster contract address configured': ContractAddresses.paymasterContract.isNotEmpty,
          'Network configuration valid': ContractAddresses.network == 'sepolia',
          'RPC endpoint available': ContractAddresses.rpcUrl.isNotEmpty,
          'Explorer URLs available': ContractAddresses.exchangeExplorerUrl.isNotEmpty,
        };
